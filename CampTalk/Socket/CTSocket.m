...
static NSString *ctSocketBaseUrl = @"ws://192.168.72.2:16233/websocket";
...
@interface CTSocket() <SRWebSocketDelegate> {
    int _index;
    NSTimer * heartBeat;
...
@implementation CTSocket

+ (CTSocket *)sharedInstance {
    static CTSocket *Instance = nil;
    static dispatch_once_t predicate;
    dispatch_once(&predicate, ^{
        Instance = [[CTSocket alloc] init];
    });
    return Instance;
}
...
- (void)openWithToken:(NSString *)token {
    ...
    [ctSocketBaseUrl stringByAppendingFormat:@"?token=%@", token];
    
    [self SRWebSocketOpenWithURLString:urlString];
}
